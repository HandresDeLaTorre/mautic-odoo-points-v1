...
const empleado = async (id) => {
    await odoo.connect();
    return await odoo.read('res.users', [id], ["name"] );
}

const createLead = async (partnerId, nameContact, mobile, userId ) => {
    await odoo.connect();
    const empleadoData = await empleado(userId)
    const empleadoName = await empleadoData[0].name
    return await odoo.create('crm.lead',{
        partner_id: partnerId, 
        name:` ${nameContact } Es una oportunidad para ${empleadoName}`,
        phone:mobile,
        priority:'2',
        user_id: userId
    })
...
const updateLeads = async (idLead) =>{
    await odoo.connect();
    return await odoo.update('crm.lead', idLead, {priority: '2', stage_id: 2});
}
