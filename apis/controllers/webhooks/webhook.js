...
const pointschange = async (req, res) => {
    ...
    try {
        ...
        const leadPartnerId = parseInt(idLeadOdoo, 10);

        // console.log('idLeadOdoo', idLeadOdoo);
        // console.log('idUsuarioOdoo', idUsuarioOdoo);

        const validation = {
            validador: points >= 15 && points <= 29 && idUsuarioOdoo.length === 0 && idLeadOdoo.length === 0,
            validadorLead: points >= 30 && points <= 44 && idUsuarioOdoo.length === 1 && idLeadOdoo.length === 0,
            validadorLeadCalificado: points >= 45 && points <= 74 && idUsuarioOdoo.length === 1 && idLeadOdoo.length === 1,
        }


        console.log('Validation: ', validation.validador, validation.validadorLead, validation.validadorLeadCalificado);
        
        if (validation.validador) {
            const contactoIdCreate = await odooApi.createContact(fullname, parametros.email, parseInt(countryId, 10), parametros.mobile);
            console.log('Este es el IF', contactoIdCreate);
            res.send({ "completo": "200", "webhook": "Contact" });
        } else if (validation.validadorLead) {

            const peticiones = turnos.push(points);
            const turno = turnos.length
            console.log('Soy el Lead//');
            // const empleados = await odooApi.empleado();
            // console.log('EMPLEADOS ', empleados);

            switch (turno) {
                case 1:
                    //console.log(`ASESOR ${turno}`);
                    const asesor1 = await odooApi.empleado(turno + 6);
                    const asesorID = await asesor1[0].id;
                    await odooApi.createLead(partnerId, fullname, parametros.mobile,  asesorID );
                    // const idlead1 = await crearLead(turno)
                    console.log(`*==> Se creo un Lead con el ID: $ para el ASESOR ${turno}`, asesor1[0].name);
                    break;
                case 2:
                    const asesor2 = await odooApi.empleado(turno + 6);
                    const asesor2ID = await asesor2[0].id;
                    await odooApi.createLead(partnerId, fullname, parametros.mobile,  asesor2ID );
                    // const idlead1 = await crearLead(turno)
                    console.log(`*==> Se creo un Lead con el ID: $ para el ASESOR ${turno}`, asesor2[0].name)
                    break; 
                case 3:
                    const asesor3 = await odooApi.empleado(turno + 6);
                    const asesor3ID = await asesor3[0].id;
                    await odooApi.createLead(partnerId, fullname, parametros.mobile,  asesor3ID );
                    // const idlead1 = await crearLead(turno)
                    console.log(`*==> Se creo un Lead con el ID: $ para el ASESOR ${turno}`, asesor3[0].name)
                    break;
                case 4:
                    const asesor4 = await odooApi.empleado(turno + 6);
                    const asesor4ID = await asesor4[0].id;
                    await odooApi.createLead(partnerId, fullname, parametros.mobile,  asesor4ID );
                    // const idlead1 = await crearLead(turno)
                    console.log(`*==> Se creo un Lead con el ID: $ para el ASESOR ${turno}`, asesor4[0].name)
                    turnos = [];
                    break;            
                default:
                    // turnos = []
            ...
            res.send({ "completo": "200", "webhook": "Lead" });
        }else if (validation.validadorLeadCalificado) {
            console.log('Soy el lead calificado//');
            await odooApi.updateLeads(leadPartnerId )
            res.send({ "completo": "200", "webhook": "Lead Calificado" });
        }else {
            console.log('Soy el defaul//');
            res.send({ "completo": "200", "webhook": "Else" });
        }
         
        
        // if (points >= 15 && points <= 29 && idUsuarioOdoo.length === 0 && idLeadOdoo.length === 0) {
        ...
        // }else{
        //     console.log('********* Fin del programa ************', points);
        // }
    // res.send({ "completo": "200", "webhook": "TEST" });
    } catch (e) {
        httpError(res, e)
